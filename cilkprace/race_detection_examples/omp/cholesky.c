 ...
 *
 */
/* OpenMP-task port of ../cholesky.c for detector comparison. */
...
#pragma omp task shared(r00)
      r00 = mul_and_subT(depth, lower, a->child[_00],
                         b->child[TR_00], r00);
    }

    if (!lower && a->child[_00] && b->child[TR_01]) {
#pragma omp task shared(r01)
      r01 = mul_and_subT(depth, 0, a->child[_00], b->child[TR_01],
                         r01);
    }

    if (a->child[_10] && b->child[TR_00]) {
#pragma omp task shared(r10)
      r10 = mul_and_subT(depth, 0, a->child[_10], b->child[TR_00],
                         r10);
    }

    if (a->child[_10] && b->child[TR_01]) {
#pragma omp task shared(r11)
      r11 = mul_and_subT(depth, lower, a->child[_10],
                         b->child[TR_01], r11);
    }
#pragma omp taskwait
...
#pragma omp task shared(r00)
      r00 = mul_and_subT(depth, lower, a->child[_01],
                         b->child[TR_10], r00);
    }

    if (!lower && a->child[_01] && b->child[TR_11]) {
#pragma omp task shared(r01)
      r01 = mul_and_subT(depth, 0, a->child[_01], b->child[TR_11],
                         r01);
    }

    if (a->child[_11] && b->child[TR_10]) {
#pragma omp task shared(r10)
      r10 = mul_and_subT(depth, 0, a->child[_11], b->child[TR_10],
                         r10);
    }

    if (a->child[_11] && b->child[TR_11]) {
#pragma omp task shared(r11)
      r11 = mul_and_subT(depth, lower, a->child[_11],
                         b->child[TR_11], r11);
    }
#pragma omp taskwait
  }
...
Matrix backsub(int depth, Matrix a, Matrix l) {
  ...
  Matrix a00 = a->child[_00];
  Matrix a01 = a->child[_01];
  Matrix a10 = a->child[_10];
  Matrix a11 = a->child[_11];

  Matrix l00 = l->child[_00];
  Matrix l10 = l->child[_10];
  Matrix l11 = l->child[_11];

  assert(l00 && l11);

  {
    if (a00) {
#pragma omp task shared(a00)
      a00 = backsub(depth, a00, l00);
    }
    if (a10) {
#pragma omp task shared(a10)
      a10 = backsub(depth, a10, l00);
    }
#pragma omp taskwait

    if (a00 && l10) {
#pragma omp task shared(a01)
      a01 = mul_and_subT(depth, 0, a00, l10, a01);
    }
    if (a10 && l10) {
#pragma omp task shared(a11)
...
int main(int argc, char *argv[]) {
  ...
  if (filename[0]) {
    ...
    while (!feof(f)) {
      ...
      int res = sscanf(buf, "%lf %lf %lf", &fr, &fc, &val);
      int r = fr;
      int c = fc;
      if (res <= 0)
        break;

...
/* OpenMP-task port of ../rectmul.c for detector comparison. */
...
static long long add_matrix(block *T, long ot, block *R, long orr, long x,
  ...
  {
    if (x > y) {
#pragma omp task shared(_tmp1)
      _tmp1 = add_matrix(T, ot, R, orr, x / 2, y);
      _tmp2 = add_matrix(T + (x / 2) * ot, ot, R + (x / 2) * orr, orr,
                         (x + 1) / 2, y);

    } else {
#pragma omp task shared(_tmp1)
      _tmp1 = add_matrix(T, ot, R, orr, x, y / 2);
...
static long long multiply_matrix(block *A, long oa, block *B, long ob, long x,
...

  {
    if ((x >= y) && (x >= z)) {
#pragma omp task shared(_tmp1)
      _tmp1 = multiply_matrix(A, oa, B, ob, x / 2, y, z, R, orr, add);
...
#pragma omp task shared(_tmp1)
        _tmp1 = multiply_matrix(A, oa, B, ob, x, y, z / 2, R, orr, add);

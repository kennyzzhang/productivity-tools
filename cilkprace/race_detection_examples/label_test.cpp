...
#include <iostream>
int main() {
  std::cerr << "Skipping Cilk OS Label Determinism Tests (nocilk or pedigrees "
               "disabled).\n";
  return 0;
}
#else
...
int verify_label(std::vector<uint8_t> expected, const char *context) {
  ...
  // This sleep is important-- it ensures steals.
  usleep(500);
  return 0;
}

std::vector<uint8_t> append_label(std::vector<uint8_t> base,
...
std::vector<uint8_t> sync_label(std::vector<uint8_t> base, int /*conts*/) {
  ...
  // effectively returning to the base label, and then increments the last
  // element by 2.
  if (!base.empty()) {
    base.back() += 2;
  }
...
int test_single_spawn(std::vector<uint8_t> base) {
  ...
  fails +=
      verify_label(append_label(base, {0}), "test_single_spawn continuation");
...
int outer_child(std::vector<uint8_t> base) {
  ...
  fails += verify_label(sync_label(append_label(base, {1}), 1),
                        "outer_child after sync");
  return fails + child_fails;
}
...
int test_nested_spawn(std::vector<uint8_t> base) {
  ...
  fails +=
      verify_label(append_label(base, {0}), "test_nested_spawn continuation");
...
int test_multiple_syncs(std::vector<uint8_t> base) {
  ...
  int cf1 = cilk_spawn verify_label(append_label(base, {1}),
                                    "test_multiple_syncs first child");
  fails += verify_label(append_label(base, {0}),
                        "test_multiple_syncs first continuation");
  cilk_sync;
  fails += cf1;
  ...
  int cf2 = cilk_spawn verify_label(append_label(after_sync_1, {1}),
                                    "test_multiple_syncs second child");
  fails += verify_label(append_label(after_sync_1, {0}),
                        "test_multiple_syncs second continuation");
  cilk_sync;
  fails += cf2;
...
int main() {
  ...
  total_fails += verify_label(append_label(base, {0}),
                              "main after test_single_spawn continuation");
  cilk_sync;
  ...
  total_fails += verify_label(append_label(base, {0}),
                              "main after test_nested_spawn continuation");
  cilk_sync;
  base = sync_label(base, 1);
  ...
  total_fails += verify_label(append_label(base, {0}),
                              "main after test_loop_spawn continuation");
  cilk_sync;
  base = sync_label(base, 1);
  ...
  total_fails += verify_label(append_label(base, {0}),
                              "main after test_multiple_syncs continuation");
  cilk_sync;
  base = sync_label(base, 1);
  ...
  total_fails += verify_label(append_label(base, {0}),
                              "main after test_cilk_scope continuation");
  cilk_sync;
  base = sync_label(base, 1);
